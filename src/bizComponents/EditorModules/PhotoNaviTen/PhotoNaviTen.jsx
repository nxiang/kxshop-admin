...
import Css from './PhotoNaviTen.module.scss';

class PhotoNaviTen extends Component {
  constructor(props) {
    super(props);
    this.state = {};
  }
        ...
        {itemData.map((item, index) => {
          return (
            <div className={Css['nav-item']} key={index}>
              ...
              {itemStyle === 2 && (
                <img
                  className={`${Css['item-circle-img']} ${itemData.length >= 10 ? Css['small'] : ''}`}
                  src={
                    item.image === ''
                      ? 'https://img.kxll.com/admin_manage/icon/decoration_default_img.png'
                      : item.image
