...
import Css from './AlterRecommend.module.scss';
...
class ItemDataRender extends Component {
  ...
  alterClassifyData(data) {
    ...
    if (difference.length + itemDataIdList.length < 99) {
    ...
    } else if (itemDataIdList.length < 99) {
      for (let i in difference) {
        for (let j in newitemDataList) {
          if (difference[i] === newitemDataList[j].itemId && dataList.length < 99) {
            dataList.push(newitemDataList[j]);
          }
        }
      }
  ...
  render() {
    ...
    const { item, itemNum, itemType, itemSubTitle, asideType } = this.props;
    console.log('itemTypes=', asideType);
      ...
      <div className={Css['item-data-render-box']}>
        ...
        <div className={Css['radio-box']}>
          ...
          {asideType !== 'integral' ? (
            ...
            <Radio.Group onChange={this.radioIntegChange.bind(this)} value={1}>
              <Radio value={1}>按商品</Radio>
            </Radio.Group>
          )}
        </div>
        <div className={Css['prompt-text']}>最多添加99个商品</div>
        <div className={Css['goods-box']}>
          ...
          {(!item.data || item.data.length < 99) && (radioValue === 1 || asideType == 'integral' )&& (
            <GoodsSelect
              itemType={asideType == 'integral' ? 1 : itemType}
              itemData={item.data}
              alterData={this.alterGoodsData.bind(this)}
            />
          )}
          {(!item.data || item.data.length < 99) && (radioValue === 2 && asideType != 'integral') && (
            <ClassifySelect
              itemType={asideType == 'integral' ? 1 : itemType}
              itemData={item.data}
...
class AlterRecommend extends Component {
  constructor(props) {
    super(props);
    const { itemData, itemSubTitle, asideType } = props;
  ...
  render() {
    ...
    console.log('asideType=', asideType);
      ...
      <div className={Css['alter-recommend-box']}>
        <div className={Css['alter-header']}>
          <div className={Css['header-left']}>
            <p className={Css['header-left-title']}>
              {asideType == 'integral' ? '积分商品' : '商品推荐'}
            </p>
